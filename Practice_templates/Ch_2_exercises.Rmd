...
> 2 is "probability of rain given Monday"
> 4 is "probaility of rain and Monday" divided by "probability of Monday" which is equivalent
...
> There is a true proportion of water on earth. The probability refers to our best guess on how much water on earth but this is just a guess and not the actual proportion of water.
...
```{r}
# making a thousand numbers evenly spaced from 0 to 1
p_grid <- seq( from=0 , to=1 , length.out=1000 )
# making the prior - they are all 1
prob_p <- rep( 1 , 1000 )
# using binomial sampling formula, number of water samples, total samples and the p_grid
prob_data <- dbinom( 3 , size=3 , prob=p_grid )
# multiplying the prior and the sample to get posterior
posterior <- prob_data * prob_p
# normalize to make it into a probability distribution
...
p_grid <- seq( from=0 , to=1 , length.out=1000 )
prob_p <- rep( 1 , 1000 )
prob_data <- dbinom( 3 , size=4 , prob=p_grid )
posterior <- prob_data * prob_p
posterior <- posterior / sum(posterior)
plot( p_grid , posterior , type="b" ,
    xlab="probability of water" , ylab="posterior probability" )
...
p_grid <- seq( from=0 , to=1 , length.out=1000 )
prob_p <- rep( 1 , 1000 )
prob_data <- dbinom( 5 , size=7 , prob=p_grid )
posterior <- prob_data * prob_p
posterior <- posterior / sum(posterior)
plot( p_grid , posterior , type="b" ,
    xlab="probability of water" , ylab="posterior probability" )
...
```{r}
# What would it look like if we had a ton of samples?
p_grid <- seq( from=0 , to=1 , length.out=10000 )
prob_p <- rep( 1 , 1000 )
prob_data <- dbinom( 750 , size=1000 , prob=p_grid )
posterior <- prob_data * prob_p
posterior <- posterior / sum(posterior)
plot( p_grid , posterior , type="b" ,
    xlab="probability of water" , ylab="posterior probability" )
...
# making the prior - if the point in the grid is less than 0.5 then 0, else 1
prior <- ifelse( p_grid < 0.5 , 0 , 1 )
# multiplying the prior and the sample to get posterior
posterior <- likelihood * prior
# normalize/standardize to make it into a probability distribution
posterior <- posterior / sum(posterior)
# plotting
